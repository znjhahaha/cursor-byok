-------0.0.47------
- 新增 AnyRouter 手动智能排队检测：发送极小流式请求，支持 429/无渠道持续排队、指数退避、Retry-After、并发限制、取消和实时等待状态
- 修复 Anthropic 缺少 message_stop、OpenAI Responses 缺少终止事件导致的假成功，并为纯文本断流提供最多两次安全续写恢复
- 将 Fable5 refusal 变为明确安全拒绝错误；为 Grok 等模型增加 Agent 模式承诺型短停单次续跑，工具调用与副作用场景不自动恢复
- 前端模型卡片增加排队次数、等待时间、倒计时、单项取消和批量停止；中转站预热开关改为仅影响手动检测
- 完善 Claude Code/Codex/generic 请求模式回归测试、i18n 校验、诊断日志与 AnyRouter 脱敏流式 smoke test
